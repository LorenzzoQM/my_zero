...
def test_select_action_from_visits():
    ...
    prediction_net = lambda x: (
        torch.from_numpy(np.array([1.0, 0, 0, 0, 0])),
        torch.tensor([0.1]),
    )
    dynamics_net = lambda s, a: (s, torch.tensor([0.1]))
    mcts = MuZeroMCTS(prediction_net, dynamics_net, num_actions=5, gamma=0.9)

    visit_counts, root_value_count, root_value, _ = mcts.run(
        root_latent=torch.tensor([0.0] * 10), num_simulations=1
    )
